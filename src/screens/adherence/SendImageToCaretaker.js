  ...
  const RenderMeds = ({item}) => {
    ...
    return (
      <View
        ...
        <TouchableOpacity id="clicked" onPress={() => clickedFnc}>
  ...
  const renderitem = ({item}) => {
    ...
    const fetchMedicines = async () => {
      db.transaction(async txn => {
        let medsArr = await queryData.getusermeds(txn);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.container1}>
        ...
        <Button title="Share" id="share" onPress={shareFnc}></Button>
      </View>
